...
- ✅ **M2 manifest 持久化已完成**（2026-07-24）：14 子项目 → ArsenalsOs fork 统一 `aos26` 分支（11 小/中仓库 `aos_*` + 3 大仓库 fork of upstream 同名：`android_frameworks_base`/`android_kernel_xiaomi_sm8450`/`proprietary_vendor_xiaomi_marble`，增量 push 绕过 110万commit size 限制）+ `local_manifests/arsenals.xml` 覆盖（remote arsenals + build/make linkfile）。`repo sync --force-sync` 从 fork 拉 aos26 不丢。顶层 los 仓无 remote 不 push（敏感安全）。详见记忆 `reference-m2-fork-persistence`。
